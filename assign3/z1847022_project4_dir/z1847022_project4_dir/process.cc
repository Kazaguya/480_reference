...
string process::toString() {
    ...
    ss << this->processID << "   ";
    //ss << this->processName << " : id# " << this->processID << " arrivalTime: " << this->arrivalTime;
    //ss << " History: { ";
    /*for (int i = 0; i < this->history.size(); i++) {
        if (i == this->history_sub) {
            ss << "=>";
        }
        ss << this->history[i].first << ":" << this->history[i].second;
        if (i < this->history.size()-1) {ss << ", ";}
    }*/
    //ss << "\n";
