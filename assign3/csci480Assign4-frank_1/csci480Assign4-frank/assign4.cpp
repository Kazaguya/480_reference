...
int main(int argc, char* argv[]){
    ...
    inFile >> processName;  
    inFile >> arrivalTime;
    //stophere the delimere, will break out of read loop once word encountered
    if (processName == "STOPHERE"){
        break;
    }

    //cout<<processName<<" "<<" proces id: "<<processes.processID<<" "<<arrivalTime<<" cpu pairs: ";
    ...
    for (int start =0; start < 10; start++){
        ...
        //cout<< cpuPairs.first <<cpuPairs.second<<" ";

        processes.history.push_back(cpuPairs);
    }

    //cout<<" "<<processes.processName<<" "<<processes.processID;
    cout<<endl;
    processes.processID++;
    ...
    while (timer <= 125){ //|| (ReadyQueue.empty() && EntryQueue.empty() && InputQueue.empty()&& OutPutQueue.empty() && cpu_active == NULL && Iactive == NULL && Oactive == NULL)){
        // //if there is no cpu_active process and the the ready queue is empty ==> idle time
        // if (cpu_active == NULL && EntryQueue.empty()){
        //     cout<<"At time "<<timer<<" , Active is 0, so we have idle time for a while"<<endl;
        //     // continue;
        // }
        if (timer % HOW_OFTEN == 0){
            ...
            if (cpu_active == NULL){
                ...
                if (ReadyQueue.empty() && !EntryQueue.empty()){
                    ...
                    for (unsigned long int start =0; start<EntryQueue.size();start++){          
                        ...
                        if( EntryQueue[start]->arrivalTime <=timer && processInPlay <=IN_USE){             
        ...
        if (cpu_active!= NULL){
            ...
            if (cpu_active->history[cpu_active->subHistory].second == 0){
                ...
                cpu_active->CPUCount++;
                cout<<"end of cpu burst at time: "<<timer<<endl;
                ...
                switch(cpu_active->history[cpu_active->subHistory].first){
		            ...
		            case 'I':
                        ...
                        cout<<"Input burst starting at time: "<<timer<<endl;
		            ...
		            //output
                    case 'O':
                        cout<<"Output burst Starting at time "<<timer<<endl;
                        ...
                        break;
                    case 'N':
                        cout<<"\n\nProcess "<<cpu_active->processID<<" has ended"<<endl;
                        cout<<"Name: " <<cpu_active->processName<<endl;
                        cout<<"Started at time: "<<cpu_active->arrivalTime<<" and ended at time "<<timer<<endl;
                        cout<<"Total CPU time = "<<cpu_active->CPUTimer<<" in "<<cpu_active->CPUCount<<" CPU bursts"<<endl;
                        cout<<"Total Input time = "<<cpu_active->InputBurstTimer<<" in "<<cpu_active->InputBurstCount<<" Input burst"<<endl;
                        cout<<"Total Output time = "<<cpu_active->OutputBurstTimer<<" in "<<cpu_active->OutputBurstCount<<" Output bursts"<<endl;   
                        cpu_active = NULL;
                        break;    
                }
	...
	if (Iactive != NULL){
            ...
            if (Iactive->history[Iactive->subHistory].second == 0){
		        ...
		        switch(Iactive->history[Iactive->subHistory].first){
                    ...
                    case 'C':
                        cout<<"CPU burst starting at time: "<<timer<<endl;
                        //cout<<Iactive->history[Iactive->subHistory].first<<" "<<Iactive->history[Iactive->subHistory].second<<endl;
        ...
        if (Oactive != NULL){
	        ...
	        if (Oactive->history[Oactive->subHistory].second ==0){
                ...
                switch(Oactive->history[Oactive->subHistory].first){
                    case 'I':
                    InputQueue.push_back(Oactive);
                    Iactive = InputQueue.front();
                    InputQueue.pop_front();
                    //Iactive->InputBurstTimer++;
                    //Iactive->history[Iactive->subHistory].second--;
        ...
        if(cpu_active == NULL && Iactive == NULL && Oactive == NULL && ReadyQueue.empty() && InputQueue.empty() && OutPutQueue.empty() && EntryQueue.empty()){
            cout<<"\n\nThe run has ended at time: "<<timer<<endl;
            break;
        }
