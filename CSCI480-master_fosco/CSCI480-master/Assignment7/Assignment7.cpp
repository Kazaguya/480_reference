/*
 ...
 /                                                               \
||  Course: CSCI-480    Assignment #: 7    Semester: Spring 2019 ||
||                                                               ||
...
||                                                               ||
||  Description:                                                 ||
||    This program simulates a File allocation table, a FAT-12   ||
||    To be exact. I strove to make the output 1 for 1 with the  ||
||    sample output using diffchecker.com (near perfect match),  ||
||    as such, the '#define HOWOFTEN 5' is left on 5 and not 6.  ||
 \_______________________________________________________________/
*/
...
bool newFile(string nameHolder, int sizeHolder)
{
	...
	if (fileNameHolder == nullptr)
	{
		if (sizeHolder != 0)
		{
			...
			/*decrement sizeHolder to prepare for loop
			 *if sizeHolder is > 0 after this, the loop
			 *will alocate more blocks
			 *else if sizeHolder is < 0, then a -1 is
			 *immediatly placed in the currentSpot 
			 *(the first open spot in FAT)
			 */
			//cout << "RCV S: " << sizeHolder;
...
Use: Modifies an exsisting file, used to change the 
	 filesize
...
Use: checks to see if the current dir size
...
Parameters: initalPost: the starting position for
                        starting the search

Returns: The position of the next zero or
        -1 if the FAT is full.
*/
...
Use: Finds a file in dir with the same name
     as the 'name' parameter

Parameters: name: a name to look for

Returns: A pointer to the Entry in dir with
         the specificed name
*/
